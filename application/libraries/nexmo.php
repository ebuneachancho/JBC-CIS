...
class nexmo
{
    ...
    public function send_message($from, $to, $message, $type = 'text')
    {
        ...
        switch ($type) {
            case 'text':
                $data =array('text' => (isset($message['text'])) ? $message['text'] : '',
                    'type' => (isset($message['type'])) ? $message['type'] : 'unicode',);
            break;
            case 'binary':
                $data = array('body' => (isset($message['body'])) ? bin2hex($message['body']) : '',
                    'udh'  => (isset($message['udh'])) ? bin2hex($message['udh']) : '',
                    'type' => (isset($message['type'])) ? $message['type'] : 'binary',);
            break;
            case 'wappush':
                $data = array('title'    => (isset($message['title'])) ? $message['title'] : '',
    ...
    public function verify_search($request_ids)
    {
        ...
        if (count($request_ids) === 1) {
            $params = array("request_id" => $request_ids,);
